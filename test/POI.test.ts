...
describe("POI", function () {
  ...
  it("supports delegated off-chain distribution minting", async () => {
    const [admin, distributor, recipient] = await ethers.getSigners();
    const Poi = await ethers.getContractFactory("POI");
    const poi = await Poi.deploy(admin.address, admin.address, 0);
    await poi.deployed();

    const distributorRole = await poi.OFFCHAIN_DISTRIBUTOR_ROLE();
    await poi.connect(admin).grantRole(distributorRole, distributor.address);

    const amount = ethers.utils.parseEther("25");
    await poi.connect(distributor).mintForOffchainDistribution(recipient.address, amount);
    expect(await poi.balanceOf(recipient.address)).to.equal(amount);

    await poi.connect(admin).revokeRole(distributorRole, distributor.address);
    await expect(
      poi.connect(distributor).mintForOffchainDistribution(recipient.address, 1)
    ).to.be.revertedWithCustomError(poi, "AccessControlUnauthorizedAccount");
  });
