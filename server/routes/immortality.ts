...
import { getUserPlan } from "../services/userPlan";
...
export function registerImmortalityRoutes(app: Express) {
  ...
  app.post("/api/immortality/activate-agent", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (plan !== "paid") {
        // Return paywall error (HTTP 200 to avoid frontend treating it as network error)
        return res.status(200).json({
          status: "error",
          code: "PAYWALL_REQUIRED",
          message: "激活 Immortality Agent 需要开通 Access Pass，请前往充值页面开通。",
        });
      }
    ...
    try {
      ...
      if (plan !== "paid") {
        ...
        return res.status(200).json({
          status: "error",
          code: "PAYWALL_REQUIRED",
          message: "铸造 Immortality 徽章需要 Access Pass，请先开通。",
        });
      }
