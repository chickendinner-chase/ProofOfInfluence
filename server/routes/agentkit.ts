...
import { hasRequiredRole, getRequestRole } from "./utils";
...
export function registerAgentKitRoutes(app: Express) {
  ...
  app.get("/api/agentkit/wallet", isAuthenticated, async (req: Request, res: Response) => {
    console.log("[AgentKit][wallet] Route hit", {
    ...
    try {
      if (!hasRequiredRole(req, ["admin"])) {
        ...
        console.log("[AgentKit][wallet] Permission denied", {
          userId: (req as any).user?.claims?.sub,
          userRole,
          required: "admin",
        });
        return res.status(403).json({ 
          ...
          error: `Current role: ${userRole || "unknown"}`,
        });
      }

      console.log("[AgentKit][wallet] Getting wallet info for admin user");
      ...
      // Get chain ID using ethers provider
  ...
  app.get("/api/agentkit/roles", isAuthenticated, async (req: Request, res: Response) => {
    console.log("[AgentKit][roles] Route hit", {
      userId: (req as any).user?.claims?.sub,
      userRole: (req as any).user?.role,
      hasAuth: !!(req as any).user,
    });
    ...
    try {
      if (!hasRequiredRole(req, ["admin"])) {
        const userRole = getRequestRole(req);
        console.log("[AgentKit][roles] Permission denied", {
          userId: (req as any).user?.claims?.sub,
          userRole,
          required: "admin",
        });
      ...
      console.log("[AgentKit][roles] Getting roles for admin user");
      const { walletProvider } = await getAgentKitContext();
  ...
  app.post("/api/agentkit/test", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Test basic operations using ethers provider
      const balance = await provider.getBalance(address);
      const network = await provider.getNetwork();
