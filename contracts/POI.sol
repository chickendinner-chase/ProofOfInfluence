...
contract POI is ERC20, ERC20Permit, ERC20Burnable, AccessControl, Pausable {
    ...
    /**
     * @notice Mints tokens for off-chain distribution flows (e.g. Stripe ledger redemptions).
     * @dev Intended for back office systems managing fiat-to-POI credits.
     * @param account Recipient of the minted tokens once fiat credits are settled.
     * @param amount Amount of tokens to mint for the recipient.
     */
    function mintForOffchainDistribution(address account, uint256 amount)
        external
        onlyRole(OFFCHAIN_DISTRIBUTOR_ROLE)
    {
        _mint(account, amount);
    }
